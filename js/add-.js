document.addEventListener('DOMContentLoaded', function () {
    ...
    function addToCart(selectedSize, productItem) {
        if (!selectedSize) {
            ...
            if (hyphenIndex !== -1) {
                ...
                if (selectedSize === '256G') {
                    priceM = productPriceString.trim();
                } else if (selectedSize === '512G') {
                    priceL = productPriceString.trim();
                }
            }
            var productPrice = '';
    
            if (selectedSize === '256G' && priceM !== '') {
                productPrice = priceM;
            } else if (selectedSize === '512G' && priceL !== '') {
                productPrice = priceL;
            } else {
    ...
    function formatPrice(price) {
        // Đảm bảo rằng `price` là số trước khi định dạng
    ...
    function calculateTotalAmount() {
        ...
        cartItems.forEach(function (item) {
            // Chuyển `item.price` sang số nếu là chuỗi
            var itemPrice = parseFloat(item.price.toString().replace(/\./g, '').replace('đ', ''));
